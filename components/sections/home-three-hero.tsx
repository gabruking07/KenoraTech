...
const cards = [
  {
    ...
    icon: Bot,
    className: "xl:-left-20 xl:top-[20rem] 2xl:-left-28"
  },
  {
    title: "Web Development",
    subtitle: "Modern & Scalable",
    icon: Globe2,
    className: "xl:-right-20 xl:top-[20rem] 2xl:-right-28"
  },
  {
    title: "Performance",
    subtitle: "99.9% Uptime",
    icon: Gauge,
    className: "xl:-left-24 xl:bottom-20 2xl:-left-32"
  },
  {
    title: "Secure & Reliable",
    subtitle: "Enterprise Grade",
    icon: ShieldCheck,
    className: "xl:-right-24 xl:bottom-20 2xl:-right-32"
  }
];
...
export function HomeThreeHero() {
  ...
  useEffect(() => {
    ...
    const applyResponsiveScene = () => {
      root.position.y = hero.clientWidth >= 1280 ? -1.08 : -0.5;
      root.scale.setScalar(hero.clientWidth >= 1280 ? 0.94 : 1);
    };
  ...
  return (
    <section
      ref={heroRef}
      className="relative isolate min-h-[820px] overflow-hidden bg-[#02040d] text-white md:min-h-[900px]"
      style={{
        "--mouse-x": "50%",
        "--mouse-y": "45%"
    ...
    >
      ...
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_var(--mouse-x)_var(--mouse-y),rgba(38,174,255,0.15),transparent_18rem),radial-gradient(circle_at_center,rgba(104,33,255,0.16),transparent_30rem),linear-gradient(180deg,rgba(2,4,13,0.32),#02040d_94%)]" />
      <div className="absolute inset-x-0 bottom-0 h-56 bg-gradient-to-t from-[#02040d] to-transparent" />

      <div className="container relative z-10 flex min-h-[820px] flex-col items-center py-16 text-center md:min-h-[900px]">
        <div className="pointer-events-none absolute left-1/2 top-[55%] hidden h-40 w-40 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full md:flex xl:top-[62%]">
          <span className="bg-gradient-to-br from-[#7eeeff] via-[#386eff] to-[#bf35ff] bg-clip-text text-[8rem] font-black leading-none text-transparent drop-shadow-[0_0_26px_rgba(36,182,255,0.9)]">
            K
          </span>
        </div>

        <div className="relative z-20 max-w-3xl px-4 sm:px-8 xl:max-w-[640px] xl:px-0">
          <span className="inline-flex rounded-full border border-[#31d6ff]/40 bg-white/5 px-4 py-2 text-xs font-bold uppercase tracking-[0.2em] text-[#75e9ff] shadow-[0_0_28px_rgba(49,214,255,0.24)] backdrop-blur-md">
            KenoraTech Digital Systems
          </span>
          <h1 className="mt-5 text-balance text-4xl font-black leading-tight tracking-normal sm:text-5xl lg:text-[3.4rem] xl:text-[2.85rem]">
            Intelligent digital products that create impact.
          </h1>
          <p className="mx-auto mt-5 max-w-2xl text-base leading-8 text-white/72 sm:text-lg xl:mt-4 xl:max-w-xl xl:text-base xl:leading-7">
            We combine strategy, design, automation and engineering to create fast, secure and scalable products for
            ambitious businesses.
          </p>
          <div className="mt-7 flex flex-col items-center justify-center gap-3 sm:flex-row xl:mt-5">
            <Link
              href="/contact"
        ...
        <div className="pointer-events-none absolute inset-0 hidden xl:block">
          {cards.map((card) => {
            ...
            return (
              <article
                key={card.title}
                className={`pointer-events-auto absolute w-[280px] rounded-[1.4rem] border border-[#7741ff]/70 bg-[#070b1b]/64 p-5 text-left shadow-[0_0_34px_rgba(72,83,255,0.28),inset_0_1px_0_rgba(255,255,255,0.12)] backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:border-[#42dfff]/80 ${card.className}`}
              >
        ...
        <div className="mt-auto grid w-full gap-4 pb-10 sm:grid-cols-2 xl:hidden">
          {cards.map((card) => {
            const Icon = card.icon;
